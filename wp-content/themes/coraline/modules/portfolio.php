...
function theme_portfolio_list( $atts, $content = null)
{
	...
	$output = '<div class="not-ajax-portfolio">'."\n";
	...
	$output .= '</ul>'."\n";
	$output .= '</nav>'."\n";
	$output .= theme_portfolio_loop($column, $cat_array, $posts_per_page, $show_title, $show_skills);
	$output .= '</div>'."\n";
...
function theme_portfolio_loop($column, $cat_array, $posts_per_page, $show_title, $show_skills) 
{
	if(empty($cat_array[0]))
	{
		$args = array( 
