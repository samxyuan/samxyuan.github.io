...
class theme_options_generator 
{
	...
	function render() 
	{
		...
		echo '<div id="icon-theme-settings" class="icon32 icon32-theme-settings"><br /></div><h2 class="theme-settings-title">' . __('Theme Settings', 'TR') . '<span>Verson:'.THEME_VERSION.'</span></h2>';
		...
		switch($type)
		{
			...
			case 'slideshow':
				...
				foreach($entries as $entry) {
					global $post;
					$options[$entry->ID] = get_meta_option('slideshow_name', $entry->ID);
				}
			break;
		}		
		return $options;
